...
def main():
    ...
    if university == "University of Newcastle":
        file = st.selectbox("Select a UON timetable", uoncals)
        uni_format = "University of Newcastle"

    if university == "University of New England":
        file = st.selectbox("Select a UNE timetable", unecals)
    ...
    if file:
        ...
        if uni_format == "University of Newcastle":
            ...
            with column2:
                comm = st.selectbox("Comms group", ("A", "B", "C", "D", "E", "F", "G", "H", "I"),
                                   index=None,
            ...
            if go and pbl and clin and comm:
                ...
                failed_event = generate_cal_une(saved, date_start, date_end, dates)

                if failed_event:
                    st.error(failed_event)
...
def process_xlsx_une(pbl, clin, comm, ws):
    ...
    # [Week, Day, Date, Time, Duration, Group, Venue, Session, Discipline, Category, Attendance, Staff]
    ...
    for row in ws.iter_rows(min_row=title_row + 1, max_row=ws.max_row, max_col=13, values_only=True):
        events.append(row)
    ...
    for i in events:
        ...
        if "ALL" in students:
            ...
            if bool(re.search(rf'(?<!\d){clin}(?!\d)', students)):
                keep = True
        elif students.startswith("Comms"):
            if bool(re.search(rf'(?<!\d){comm}(?!\d)', students)):
                keep = True
        elif i[8] == "Anatomy":
            keep = True
        elif students.startswith("TBA"):
            keep = True
        elif "groups" in students:
            keep = True
        elif students == pbl:
            keep = True
        ...
        if "Zoom" in str(i[6]):
            try:
                i[6] = ws.cell(row=row_num, column=8).hyperlink.target
...
def generate_cal_une(events, date_start, date_end, dates):
    ...
    with st.status("Importing events..."):
        if not dates:
            for i in events:
                try:
                    if date_start - datetime.timedelta(days=1) < i[2].date() < date_end + datetime.timedelta(days=1):
                        event = Event()
                        no_time = False

                        try:
                            start, end = convert_datetime(i[2], i[3])
                        ...
                        event.add('summary', i[7])
                        event.add("description", desc)
                        cal.add_component(event)
                        if not no_time:
                            st.write(f"{start}: " + i[7].replace('\n', ' '))
                        else:
                            st.write(i[7].replace('\n', ' '))
                except AttributeError:
                    failed_event.append(f"{i[7]}: event import failed, check spreadsheet.")
